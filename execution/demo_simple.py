"""
Simple Demo of Generic Block System
Shows the core concept without external dependencies
"""
...
class SimpleGenericBlockExecutor:
    """Simplified Generic Block Executor for demonstration"""
...
async def demo_windows_process_monitoring():
    ...
    executor = SimpleGenericBlockExecutor()
    ...
    print(f"    📄 Output: {check_result['result']['stdout']}")
...
result = {
    ...
    'maintenance_completed_at': datetime.datetime.now().isoformat()
}
            """
        },
        {"results": results}
...
result = {
    ...
    alert_condition = await executor.execute_block(
        ...
        {
            "id": "check-alerts",
            "condition": "{{data.alerts.high_cpu}} or {{data.alerts.stopped}}"
        },
...
result = {
    ...
    'maintenance_time': datetime.datetime.now().isoformat(),
    'status': 'completed'
}
            """
        }
