                    ...
                    for target_row in target_rows:
                        ...
                        service_rows = await conn.fetch("""
                            SELECT id, target_id, service_type, port, is_default, is_secure, 
                                   is_enabled, notes, credential_type, created_at, updated_at
                            FROM assets.enhanced_target_services 
                            WHERE target_id = $1 
                ...
                async with self.db.pool.acquire() as conn:
                    async with conn.transaction():
                        ...
                        for service in target_data.services:
                            ...
                            await conn.execute("""
                                INSERT INTO assets.enhanced_target_services 
                                (target_id, service_type, port, is_default, is_secure, is_enabled, notes,
                                 credential_type, username, password, private_key, public_key, api_key, 
                                 bearer_token, certificate, passphrase, domain)
            ...
            try:
                async with self.db.pool.acquire() as conn:
                    async with conn.transaction():
                        ...
                        await conn.execute(
                            "DELETE FROM assets.enhanced_target_services WHERE target_id = $1", target_id
                        )
                        
                        # Delete target
                        result = await conn.execute(
                            "DELETE FROM assets.enhanced_targets WHERE id = $1", target_id
                        )
            ...
            try:
                async with self.db.pool.acquire() as conn:
                    ...
                    service_row = await conn.fetchrow("""
                        SELECT ets.*, et.name as target_name, et.hostname, et.ip_address
                        FROM assets.enhanced_target_services ets
                        JOIN assets.enhanced_targets et ON ets.target_id = et.id
                        WHERE ets.id = $1 AND ets.target_id = $2
                    ...
                    if not host:
                        raise HTTPException(status_code=400, detail="No host or IP address configured")
            ...
            try:
                async with self.db.pool.acquire() as conn:
                    ...
                    for group in groups:
                        ...
                        group_list.append(TargetGroup(**group_dict))
                    
                    return TargetGroupListResponse(groups=group_list, total=total)
            except Exception as e:
            ...
            try:
                async with self.db.pool.acquire() as conn:
                    ...
                    """, group_data.name, group_data.description, group_data.parent_group_id,
                         path, level, group_data.color, group_data.icon)
            ...
            try:
                async with self.db.pool.acquire() as conn:
                    ...
                    if include_descendants:
                        ...
                        targets_query = """
                            SELECT DISTINCT et.id, et.name, et.hostname, et.ip_address, et.os_type, et.os_version,
                                   et.description, et.tags, et.created_at, et.updated_at
                            FROM assets.enhanced_targets et
                            JOIN assets.target_group_memberships tgm ON et.id = tgm.target_id
                            JOIN assets.target_groups tg ON tgm.group_id = tg.id
                            WHERE tg.path LIKE $1
                        ...
                        targets_query = """
                            SELECT et.id, et.name, et.hostname, et.ip_address, et.os_type, et.os_version,
                                   et.description, et.tags, et.created_at, et.updated_at
                            FROM assets.enhanced_targets et
                            JOIN assets.target_group_memberships tgm ON et.id = tgm.target_id
                    ...
                    async with conn.transaction():
                        ...
                        for target_id in target_ids:
                            ...
                            target_exists = await conn.fetchval(
                                "SELECT EXISTS(SELECT 1 FROM assets.enhanced_targets WHERE id = $1)",
                                target_id
                            )
