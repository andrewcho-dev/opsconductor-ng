...
async def get_celery_status() -> Dict[str, Any]:
    """Get Celery system status and health information"""
    try:
        ...
        return {
            ...
            "tasks": {
                ...
                "registered": sum(len(tasks) for tasks in registered_tasks.values())
            },
            "queues": queue_stats,
            "timestamp": datetime.utcnow().isoformat()
        }
        ...
        logger.error(f"Celery status retrieval error: {e}")
        return {
            "status": "error",
            "error": str(e),
            "workers": {"total": 0, "online": 0, "offline": 0},
            "tasks": {"active": 0, "registered": 0},
            "queues": {"queued_steps": 0, "running_steps": 0, "succeeded_steps": 0, "failed_steps": 0},
            "timestamp": datetime.utcnow().isoformat()
        }

@app.get("/celery/metrics")
async def get_celery_metrics() -> Dict[str, Any]:
    """Get detailed Celery performance metrics"""
    try:
        from shared.celery_config import celery_app
        ...
        logger.error(f"Celery metrics retrieval error: {e}")
        return {
            "workers": {},
            "performance": {"total_processed": 0, "hourly_stats": []},
            "error": str(e),
            "timestamp": datetime.utcnow().isoformat()
        }

@app.get("/celery/queues")
async def get_celery_queue_info() -> Dict[str, Any]:
    """Get detailed Celery queue information"""
    try:
        from shared.celery_config import celery_app
        ...
        for worker_name in set(list(reserved.keys()) + list(active.keys())):
            worker_reserved = reserved.get(worker_name, [])
            worker_active = active.get(worker_name, [])
            
            queue_info[worker_name] = {
                "queued": len(worker_reserved),
                "active": len(worker_active),
                "queued_tasks": [task.get('name', 'unknown') for task in worker_reserved],
                "active_tasks": [task.get('name', 'unknown') for task in worker_active]
            }
            
            total_queued += len(worker_reserved)
            total_active += len(worker_active)
        
        # Get database queue statistics with more detail
        with get_db_cursor(commit=False) as cursor:
            cursor.execute("""
                SELECT 
                    type,
                    ...
                    COUNT(*) FILTER (WHERE status = 'failed') as failed,
                    AVG(EXTRACT(EPOCH FROM (finished_at - started_at))) FILTER (WHERE status IN ('succeeded', 'failed')) as avg_duration
                FROM job_run_steps
                WHERE created_at > NOW() - INTERVAL '24 hours'
                GROUP BY type
                ORDER BY queued DESC, running DESC
            """)
            
            step_type_stats = [dict(row) for row in cursor.fetchall()]
        
        return {
            "summary": {
                "total_queued": total_queued,
                "total_active": total_active,
                "workers": len(queue_info)
            },
            "workers": queue_info,
            "step_types": step_type_stats,
            "timestamp": datetime.utcnow().isoformat()
        }
        
    except Exception as e:
        logger.error(f"Celery queue info retrieval error: {e}")
        return {
            "summary": {"total_queued": 0, "total_active": 0, "workers": 0},
            "workers": {},
            "step_types": [],
            "error": str(e),
            "timestamp": datetime.utcnow().isoformat()
