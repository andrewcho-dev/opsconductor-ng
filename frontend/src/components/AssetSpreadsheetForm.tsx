...
const AssetSpreadsheetForm: React.FC<AssetSpreadsheetFormProps> = ({ asset, onSave, onCancel, mode }) => {
  ...
  const fieldDefinitions: FieldDefinition[] = [
    ...
    { 
      ...
      required: true, 
      section: 'Primary Communication Service'
    },
    { 
      field: 'port', 
      label: 'Port', 
      ...
      validation: (value) => {
        ...
        return port < 1 || port > 65535 ? 'Port must be between 1 and 65535' : null;
      }
    },
    { 
      field: 'is_secure', 
    ...
    { 
      ...
      type: 'dropdown', 
      options: ['username_password', 'ssh_key', 'api_key', 'bearer_token', 'certificate'], 
    ...
    { 
      ...
      conditional: (formData) => formData.credential_type === 'certificate'
    },
    { 
      field: 'passphrase', 
      label: 'Passphrase', 
      type: 'password', 
      section: 'Primary Service Credentials',
      placeholder: 'Enter passphrase for encrypted keys/certificates',
      conditional: (formData) => ['ssh_key', 'certificate'].includes(formData.credential_type)
    },
    { 
      field: 'domain', 
      label: 'Domain', 
      type: 'text', 
      section: 'Primary Service Credentials',
      placeholder: 'Windows domain for authentication',
      conditional: (formData) => formData.credential_type === 'username_password'
    },
