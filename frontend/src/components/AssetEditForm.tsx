...
import { X, Plus, Trash2, Eye, EyeOff, Server, Shield, Globe, Database, Mail, HardDrive } from 'lucide-react';
...
const AssetEditForm: React.FC<AssetEditFormProps> = ({ asset, onClose, onSuccess }) => {
  const [formData, setFormData] = useState<AssetEditData>({
    name: '',
    hostname: '',
    ip_address: '',
    description: '',
  ...
  const fetchAssetDetails = async () => {
    try {
      ...
      setFormData({
        ...
        hostname: assetDetails.hostname || '',
        ip_address: assetDetails.ip_address || '',
        description: assetDetails.description || '',
        tags: assetDetails.tags || [],
        os_type: assetDetails.os_type || 'linux',
        os_version: assetDetails.os_version || '',
        service_type: assetDetails.service_type || 'ssh',
        port: assetDetails.port || 22,
        is_secure: assetDetails.is_secure || false,
        credential_type: assetDetails.credential_type || '',
        username: assetDetails.username || '',
        domain: assetDetails.domain || '',
        password: '', // Never pre-fill passwords
        private_key: '', // Never pre-fill private keys
  ...
  const handleInputChange = (field: keyof AssetEditData, value: any) => {
